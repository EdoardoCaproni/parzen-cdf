...
target slightly. We pursue this deliberately in the follow-up study below (Section~\ref{sec:next}),
rather than simply running more distributions.

\section{Follow-up: can the network surpass the kernel estimate?}
\label{sec:next}

\emph{This is a follow-up phase, separate from the naive experiments above.} Those established that a
well-trained network only \emph{copies} its kernel target. Here we test whether two techniques --- both
still training only on the data points --- can make it \emph{beat} the target. The opening is that each
label $\Fhat(x_i)$ is a \emph{noisy} estimate of the true $F(x_i)$: a model that captures the smooth
underlying trend, rather than the finite-sample wiggle, can average that noise away. We average over
$8$ seeds and report a spread, because the effect is small and must be told apart from seed noise.

\paragraph{What failed: smoothing the network directly.} The obvious idea --- regularise the network
to be smoother --- does not work here. Plain weight decay was \emph{catastrophic} (it pulls the output
toward the flat value $\tfrac12$, the wrong shape for a CDF; the gap blew up to $0.5$). A
CDF-appropriate curvature penalty (penalising $|\mathrm{d}^2F/\mathrm{d}x^2|$ at the data points) was
neutral-to-slightly-harmful. The reason: the small network is \emph{already} smooth enough on its own,
so an explicit smoothness prior only adds bias. This is a clean negative result.

\paragraph{What worked: ensembling, and a sharper bandwidth.} Averaging several networks (trained from
different random seeds) reliably beats the Silverman kernel ceiling --- more so at small sample sizes,
exactly the noise-averaging effect predicted. And the combination that matters: training that ensemble
on a \emph{sharper-than-default} bandwidth, whose lower bias the ensemble's variance-reduction then
cleans up. At $n=500$ this \textbf{beats even the best data-driven kernel} (the adaptive bandwidth) on
all three distributions, in the mean and with much smaller variance (Table~\ref{tab:followup},
Figure~\ref{fig:followup}).

\begin{table}[h]
  \centering
  \caption{Follow-up at $n=500$: CDF gap (mean $\pm$ std over $8$ seeds). Smoothing the network does
  not help; an ensemble on a sharper bandwidth beats even the best data-driven kernel.}
  \begin{tabular}{lccc}
    \toprule
    Method & single Gaussian & symmetric mixture & asymmetric mixture \\
    \midrule
    Parzen, Silverman (ceiling)        & $0.031 \pm 0.012$ & $0.079 \pm 0.010$ & $0.047 \pm 0.009$ \\
    Parzen, adaptive (\textbf{best kernel}) & $0.025 \pm 0.011$ & $0.046 \pm 0.012$ & $0.028 \pm 0.010$ \\
    network, plain                     & $0.033 \pm 0.009$ & $0.081 \pm 0.010$ & $0.048 \pm 0.010$ \\
    network + curvature penalty        & $0.035 \pm 0.012$ & $0.083 \pm 0.011$ & $0.050 \pm 0.011$ \\
    ensemble (default bandwidth)       & $0.030 \pm 0.003$ & $0.070 \pm 0.006$ & $0.042 \pm 0.002$ \\
    \textbf{ensemble + sharper bandwidth} & $\mathbf{0.017 \pm 0.002}$ & $\mathbf{0.035 \pm 0.006}$ & $\mathbf{0.024 \pm 0.002}$ \\
    \bottomrule
  \end{tabular}
  \label{tab:followup}
\end{table}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{followup_denoising.png}
  \caption{Follow-up at $n=500$. Dashed line is the Silverman ceiling, dotted line the best
  data-driven kernel. Smoothing bars (plain, curvature) sit at the ceiling; the ensemble on a sharper
  bandwidth (rightmost) sits below even the best-kernel line on all three distributions.}
  \label{fig:followup}
\end{figure}

\paragraph{Reading.} This is the first concrete evidence the network adds value beyond \emph{any}
single-bandwidth kernel: the sharper bandwidth lowers the bias, the ensemble removes the resulting
variance, and the two together beat the best kernel one could pick from the data alone. The advantage
is a small-sample phenomenon --- at $n=2000$ the same combination still beats the default kernel but no
longer the adaptive one, because with abundant data the kernel is already near-optimal and leaves
little for the network to add.

\section{Remaining work}

Two threads remain. First, the sharper bandwidth above was a \emph{fixed} multiple of Silverman's
($0.5\times$); the clean, fully data-driven version pairs the ensemble with a cross-validated or
adaptive bandwidth chosen from the samples. Second, the sharper distributions (a three-peak mixture, a
narrow spike inside a broad mode, and arbitrary random mixtures) are still to be studied, carrying the
ensemble-plus-sharper-bandwidth recipe --- and they are where network capacity and the monotonicity
constraint may finally start to matter.

\paragraph{Reproducibility.} Every figure and number is regenerated by the scripts in
\texttt{scripts/experiments/}: the naive experiments (\texttt{e0\_naive\_baseline.py},
\texttt{stage\_a\_single\_gaussian.py}, \texttt{stage\_b\_simple\_mixtures.py}) and this follow-up
(\texttt{followup\_denoising.py}). A fuller running log, with the exact per-setting tables, lives in
